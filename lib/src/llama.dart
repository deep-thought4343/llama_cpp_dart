...
class Llama {
  ...
  /// Directory containing ggml backend DLLs (e.g. ggml-cpu-*.dll).
  /// When set on Windows, [_initializeLlama] will explicitly load each
  /// backend DLL via [ggml_backend_load] instead of relying on
  /// [ggml_backend_load_all], which only searches standard DLL directories.
  static String? backendSearchDir;
  ...
  void _initializeLlama(
    ...
    if (!Platform.isAndroid) {
      if (backendSearchDir != null) {
        // Load backend DLLs explicitly from a custom directory.
        // ggml_backend_load_all() only searches standard DLL paths,
        // so we iterate the directory and load each backend ourselves.
        final dir = Directory(backendSearchDir!);
        if (dir.existsSync()) {
          for (final f in dir.listSync().whereType<File>()) {
            final name = f.path.split(Platform.pathSeparator).last.toLowerCase();
            if (name.startsWith('ggml-') && name.endsWith('.dll') && name != 'ggml.dll' && name != 'ggml-base.dll') {
              final pathPtr = f.path.toNativeUtf8().cast<Char>();
              lib.ggml_backend_load(pathPtr);
              malloc.free(pathPtr);
            }
          }
        }
    ...
    if (_verbose) {
      ...
      print('llama system info: $sysInfo');
    }
